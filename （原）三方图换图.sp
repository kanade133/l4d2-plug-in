...
public OnPluginStart()
{
	...
	VotensHpED = CreateConVar("l4d_VotenshpED", "1", " 启用、关闭 投票回血功能", FCVAR_PLUGIN);
	VotensHpFlag = CreateConVar("l4d_VotenshpFlag", "1", " 限制投票回血", FCVAR_PLUGIN);
	VotensMapED = CreateConVar("l4d_VotensmapED", "1", " 启用、关闭 投票换图功能", FCVAR_PLUGIN);
	VotensKickED = CreateConVar("l4d_VotenskickED", "1", " 启用、关闭 投票踢人功能", FCVAR_PLUGIN);
	VotensED = CreateConVar("l4d_Votens", "1", " 启用、关闭 插件", FCVAR_PLUGIN);
}
...
public OnMapStart()
{
	SetConVarInt(FindConVar("l4d_VotenshpFlag"), 1);
}
...
{ 
	if(GetConVarInt(VotensED) == 1)
	{
		...
		if (VotensHpE_D == 0 || GetConVarInt(VotensHpFlag) == 0)
		{
			DrawPanelItem(menu, "禁用投票回血");
		}
		else if (VotensHpE_D == 1 && GetConVarInt(VotensHpFlag) == 1)
		{
			DrawPanelItem(menu, "投票回血(每关限一次)");
		}
		if (VotensMapE_D == 0)
		{
		...
		else
		{
			...
			/*
				...
				*/
				...
				{
					AnyHp();
					SetConVarInt(FindConVar("l4d_VotenshpFlag"), 0);
...
public AnyHp()
{
	PrintToChatAll("\x03[提示]\x04所有玩家回血 本关已禁用投票回血");
