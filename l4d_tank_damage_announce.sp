...
new             Float:  g_fMaxTankHealth            = 8000.0;
...
CalculateTankHealth()
{
		new String:diffculty[64];
		g_hCvarDifficulty = FindConVar("z_difficulty");
		GetConVarString(g_hCvarDifficulty, diffculty, 64);
		if (strcmp(diffculty, "Easy") == 0) g_fMaxTankHealth = 3000.0;
		else if (strcmp(diffculty, "Normal") == 0) g_fMaxTankHealth = 4000.0;
		else if (strcmp(diffculty, "Hard") == 0) g_fMaxTankHealth = 8000.0;
		else if (strcmp(diffculty, "Impossible") == 0) g_fMaxTankHealth = 8000.0;
        else g_fMaxTankHealth = 8000.0;
...
PrintRemainingHealth()
{
        ...
        PrintToChatAll("[SM] Tank (%s) 剩余 %d 血", name, g_iLastTankHealth);
}
...
PrintTankDamage()
{
		//CalculateTankHealth();
        if (!g_bEnabled) return;
        PrintToChatAll("\x04[SM] \x05造成坦克伤害:\x03%d", RoundToFloor(g_fMaxTankHealth));
		PrintToChatAll("\x05%s\t\x04%s%\x01\t%s", "造成伤害", "伤害百分比", "玩家");
        new client;
        ...
        for (new i; i <= survivor_index; i++)
        {
                ...
                PrintToChatAll("\x05%7d\t%3s\x04[%d%%]\x01\t\t%N", damage, "", percent_damage, client);
        }
		g_fMaxTankHealth = 0.0;
}
